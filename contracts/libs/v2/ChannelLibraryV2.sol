...
import "./SignatureValidatorV2.sol";

library ChannelLibraryV2 {
	...
	function hash(Channel calldata channel)
		internal
		view
		returns (bytes32)
	{
	...
	function isSignedBySupermajority(Channel calldata channel, bytes32 toSign, bytes32[3][] calldata signatures) 
		internal
		pure
		returns (bool)
	{
		...
		for (uint i=0; i<sigLen; i++) {
			...
			if (SignatureValidatorV2.isValidSignature(toSign, channel.validators[i], signatures[i])) {
				signs++;
			} else if (i == 0) {
